                ...
                <>
                  {selectedBudget ? (
                    <div>
                      <div className="flex justify-between items-center mb-4">
                        <div>
                          <h3 className="text-lg font-medium">{selectedBudget.title || '未命名預算'}</h3>
                          <p className="text-gray-500">
                            總預算: {(selectedBudget.totalAmount || 0).toLocaleString()} {selectedBudget.currency || ''}
                            {(selectedBudget.extraBudget || 0) > 0 && 
                              ` (含額外預算 ${(selectedBudget.extraBudget || 0).toLocaleString()} ${selectedBudget.currency || ''})`
                            }
                          </p>
                        </div>
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg mb-6">
                        <h4 className="font-medium mb-2">總預算使用情況</h4>
                        {(() => {
                          try {
                            const totalSpent = selectedBudget && selectedBudget.categories && Array.isArray(selectedBudget.categories) ? 
                              selectedBudget.categories.reduce(
                                (sum, category) => sum + (Number(category.spentAmount) || 0), 0
                              ) : 0;
                            const totalBudget = Number(selectedBudget.totalAmount || 0) + Number(selectedBudget.extraBudget || 0);
                            ...
                            return (
                              <div>
                                <div className="flex justify-between mb-1">
                                  <span>{totalSpent.toLocaleString()} / {totalBudget.toLocaleString()} {selectedBudget.currency || ''}</span>
                            ...
                            console.error('計算總預算使用情況時出錯:', error);
                            return <div className="text-red-500">計算預算使用情況時發生錯誤</div>;
                          }
                        })()}
                      </div>
                      
                      <div>
                        <h4 className="font-medium mb-4">預算分類</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {selectedBudget && selectedBudget.categories && Array.isArray(selectedBudget.categories) && selectedBudget.categories.length > 0 ? (
                            selectedBudget.categories.map(category => {
                              try {
                                if (!category || !category.id) {
                                  console.error('無效的類別資料:', category);
                                  return null;
                                }
                                ...
                                return (
                                  <div key={category.id} className="border rounded-lg p-4">
                                    <div className="flex justify-between items-center mb-2">
                                      <div className="flex items-center">
                                        <span className="font-medium mr-2">
                                          {category.type ? getBudgetCategoryName(category.type) : '未知類別'}
                                        </span>
                                        {percentage >= 100 && (
                                          <Tag color="red">超出預算</Tag>
                                        )}
                                        {percentage >= 80 && percentage < 100 && (
                                          <Tag color="orange">接近預算上限</Tag>
                                        )}
                                      </div>
                                      <Button 
                                        type="text" 
                                    ...
                                    <div className="mb-2">
                                      <div className="flex justify-between mb-1">
                                        <span>
                                          {(category.spentAmount || 0).toLocaleString()} / {(category.amount || 0).toLocaleString()} {selectedBudget.currency || ''}
                                        </span>
                                        <span>{percentage}%</span>
                                      </div>
                                      <Progress 
                                    ...
                                    {category.note && (
                                      <p className="text-gray-500 text-sm mt-2">{category.note}</p>
                                    )}
                                  </div>
                                );
                              } catch (error) {
                                console.error('渲染類別時出錯:', error, category);
                                return (
                                  <div key={category.id || 'error'} className="border border-red-300 rounded-lg p-4 bg-red-50">
                                    <p className="text-red-500">渲染此類別時發生錯誤</p>
                                  </div>
                                );
                              }
                            })
